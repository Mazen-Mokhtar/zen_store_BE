...
export class GameService {
  ...
  async addGame(
    ...
    if (body.type === GameType.STEAM && (!body.price || body.price <= 0)) {
      throw new BadRequestException(
        'Price is required and must be greater than 0 for Steam games',
      );
    }
    ...
    if (body.type !== GameType.STEAM) {
      if (body.images && body.images.length > 0) {
        throw new BadRequestException(
          'Multiple images are only allowed for Steam games',
        );
      }
      if (body.video) {
      ...
      if (body.backgroundImage) {
        throw new BadRequestException(
          'Background image is only allowed for Steam games',
        );
      }
    }

    const game = await this.gameRepository.findOne({ name: body.name });
    if (game) throw new BadRequestException(messageSystem.game.alreadyExist);
    ...
    const totalFiles =
      ...
      (files?.backgroundImage?.length || 0);

    if (totalFiles > 1 && body.type !== GameType.STEAM) {
      throw new BadRequestException(
        'Multiple files upload is only allowed for Steam games',
      );
    }
    ...
    if (files && totalFiles > 0) {
      ...
      const folder = {
        folder: `${process.env.APP_NAME}/games/photos/${folderId}`,
      };
      ...
      if (files.images && files.images.length > 0) {
        try {
          ...
          throw new BadRequestException(
            `Failed to upload additional images: ${error.message}`,
          );
        }
      }
  ...
  async updateGame(user: TUser, body: UpdateGameDto, gameId: Types.ObjectId) {
    ...
    if (!game) throw new NotFoundException(messageSystem.game.notFound);
  ...
  async uploadGameImage(
    ...
    return {
      success: true,
      data: { image, message: messageSystem.game.updatedSuccessfully },
    };
  }
  async deletedGame(user: TUser, gameId: Types.ObjectId) {
    ...
    await this.gameRepository.updateOne(
      { _id: gameId },
      { isDeleted: true, isActive: false, updateBy: user._id },
    );
  ...
  async toggleGameStatus(gameId: Types.ObjectId, isActive: boolean) {
    ...
    return {
      success: true,
      data: `Game is now ${isActive ? 'active' : 'inactive'}`,
    };
  }
  ...
  async toggleGamePopular(gameId: Types.ObjectId) {
    ...
    return {
      success: true,
      data: `Game is now ${newPopularStatus ? 'popular' : 'not popular'}`,
    };
  }
  async listGames(query: {
    ...
    const games = await this.gameRepository.find(
      ...
      {
        select:
          'name description image type price isOffer originalPrice finalPrice discountPercentage offer categories isActive isPopular createdAt accountInfoFields',
      },
      {
        sort: { createdAt: -1 },
      },
