...
@Injectable()
export class IsValidAccountInfoConstraint
  ...
  async validate(accountInfo: AccountInfoDTO[], args: ValidationArguments) {
    ...
    if (!game || !game.accountInfoFields) {
      this.validationErrors.push(
        'Game not found or has no account info fields',
      );
      return false;
    }
    ...
    if (invalidEmails.length > 0) {
      this.validationErrors.push(
        `Invalid email format for fields: ${invalidEmails.join(', ')}`,
      );
      return false;
    }
    ...
    if (invalidFields.length > 0) {
      this.validationErrors.push(
        `Invalid fields that should not exist: ${invalidFields.join(', ')}`,
      );
      return false;
    }
