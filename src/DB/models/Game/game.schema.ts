...
export class Game {
  ...
  @Prop({ type: String, enum: GameType, required: true, default: GameType.GAMES })
  type: GameType;
...
GameSchema.pre('save', function(next) {
  if (this.type === GameType.STEAM && (this.price === undefined || this.price === null)) {
...
GameSchema.pre('updateOne', function(next) {
  ...
  if (update && update.type === GameType.STEAM && (update.price === undefined || update.price === null)) {
    return next(new Error('Price is required for Steam games'));
  }
  ...
  if (update && update.type === GameType.STEAM && update.isOffer) {
    ...
    // Calculate discount percentage
    update.discountPercentage = ((update.originalPrice - update.finalPrice) / update.originalPrice) * 100;
    update.discountPercentage = Math.round(update.discountPercentage * 100) / 100;
